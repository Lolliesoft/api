...
var PayerrLoader = {
	...
	createIframe: function(data) {
		...
		iframe.setAttribute('src', ( this.getParam('https') == 1 ? 'https' : 'http' ) +'://'+ this.getParam('domain', 'api.Payerr.com') + '/button/view/?'+ this.encodeData(data));
		iframe.setAttribute('class', 'PayerrButton');
	...
	init: function() {
		...
		try
		{
			...
			for (var i = (scripts.length - 1); i >= 0; i--) {
				...
				var re = new RegExp('^(http(?:s?))://(api\.(?:.*\.?)Payerr\.(?:com|local))', 'i');
	...
	loadButton: function(elm) {
		...
		if ( ((dataValue = elm.getAttribute('rev')) !== null && (dataValue.substring(0, 6) == 'Payerr')) || 
				((dataValue = elm.getAttribute('rel')) !== null && (dataValue.substring(0, 6) == 'Payerr')) ) {
		...
		else
		{
			for (field in this.validButtonParams) {
				...
				if ( (dataValue = elm.getAttribute(this.getParam('html5-key-prefix', 'data-Payerr') + '-' + this.validButtonParams[field])) !== null ) {
	...
	render: function(options, target, position) {
		...
		try
		{
			if (options) {
				...
				if (window['Payerr_uid']) { data.uid = Payerr_uid; }
				if (window['Payerr_url']) { data.url = Payerr_url; }
				if (window['Payerr_btn']) { data.button = Payerr_btn; }
				if (window['Payerr_hide']) { data.hidden = (Payerr_hide == true ? 1 : 0); }
				if (window['Payerr_cat']) { data.category = Payerr_cat; }
				if (window['Payerr_tag']) { data.tags = Payerr_tag; }
				if (window['Payerr_lng']) { data.language = Payerr_lng; }
				if (window['Payerr_tle']) { data.title = Payerr_tle; }
				if (window['Payerr_dsc']) { data.description = Payerr_dsc; }
	...
	setup: function() {
		...
		for(var i = (btns.length - 1); i >= 0 ; i--) {
			if (/PayerrButton/.test(btns[i].className)) {
